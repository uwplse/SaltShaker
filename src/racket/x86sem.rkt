...
(define minus
  fun n m -> Big.max Big.zero (Big.sub n m))
|#
...
#| 
(define pred fun n -> Big.max Big.one (Big.pred n))
|#
...
#|
(define sub fun n m -> Big.max Big.one (Big.sub n m))
...
(define compare fun x y -> Big.compare_case Eq Lt Gt x y)
           ...
           (match (@ compare a~ b~)
              ...
              ((Gt)
                (match (@ ggcdn n0 (@ sub a~ b~) b)
                   ((Pair g p)
                     (match p
                        ((Pair ab bb) `(Pair ,g ,`(Pair ,(@ add bb `(XO ,ab))
                          ,bb)))))))))
           (lambda (b0)
           (match (@ ggcdn n0 a b0)
              ((Pair g p)
...
(define succ0 Big.succ)

#|
(define pred0 fun n -> Big.max Big.zero (Big.pred n))
|#
...
#|
(define sub0 fun n m -> Big.max Big.zero (Big.sub n m))
|#

(define mul0 Big.mult)

(define compare0 Big.compare_case Eq Lt Gt)
...
(define pos_div_eucl (lambdas (a b)
  (Big.positive_case
     ...
     (lambda (_)
     (Big.n_case
        (lambda (_) `(Pair ,`(N0) ,`(Npos
        ,`(XH))))
        (lambda (p)
        (Big.positive_case
           (lambda (p0) `(Pair ,`(N0) ,`(Npos
           ,`(XH))))
           (lambda (p0) `(Pair ,`(N0) ,`(Npos
           ,`(XH))))
           (lambda (_) `(Pair ,`(Npos ,`(XH))
           ,`(N0)))
           p))
        b))
     a)))
...
(define div fun a b -> if Big.eq b Big.zero then Big.zero else Big.div a b)
...
(define ggcd0 (lambdas (a b)
  (Big.n_case
     (lambda (_) `(Pair ,b ,`(Pair ,`(N0) ,`(Npos
     ,`(XH)))))
     (lambda (p)
     (Big.n_case
        (lambda (_) `(Pair ,a ,`(Pair ,`(Npos ,`(XH))
        ,`(N0))))
        (lambda (q)
        (match (@ ggcd p q)
           ((Pair g p0)
             (match p0
                ((Pair aa bb) `(Pair ,`(Npos ,g) ,`(Pair ,`(Npos ,aa) ,`(Npos
                  ,bb))))))))
        b))
     a)))
...
(define succ1 Big.succ)

(define pred1 Big.pred)

(define sub1 Big.sub)

(define mul1 Big.mult)
...
(define compare1 Big.compare_case Eq Lt Gt)
        ...
        (match (@ pos_div_eucl0 a~ `(Zpos ,b~))
           ((Pair q r)
             (Big.z_case
                (lambda (_) `(Pair ,(opp q)
                ,`(Z0)))
                (lambda (p) `(Pair ,(opp (@ add1 q `(Zpos ,`(XH))))
                ,(@ add1 b r)))
                (lambda (p) `(Pair ,(opp (@ add1 q `(Zpos ,`(XH))))
                ,(@ add1 b r)))
                r))))
        ...
        (match (@ pos_div_eucl0 a~ b)
           ((Pair q r)
             (Big.z_case
                (lambda (_) `(Pair ,(opp q)
                ,`(Z0)))
                (lambda (p0) `(Pair ,(opp (@ add1 q `(Zpos ,`(XH))))
                ,(@ sub1 b r)))
                (lambda (p0) `(Pair ,(opp (@ add1 q `(Zpos ,`(XH))))
                ,(@ sub1 b r)))
                r))))
...
(define ggcd1 (lambdas (a b)
  (Big.z_case
     (lambda (_) `(Pair ,(abs b) ,`(Pair ,`(Z0)
     ,(sgn b))))
     (lambda (a0)
     (Big.z_case
        (lambda (_) `(Pair ,(abs a) ,`(Pair ,(sgn a)
        ,`(Z0))))
        (lambda (b0)
        (match (@ ggcd a0 b0)
           ((Pair g p)
             (match p
                ((Pair aa bb) `(Pair ,`(Zpos ,g) ,`(Pair ,`(Zpos ,aa) ,`(Zpos
                  ,bb))))))))
        (lambda (b0)
        (match (@ ggcd a0 b0)
           ((Pair g p)
             (match p
                ((Pair aa bb) `(Pair ,`(Zpos ,g) ,`(Pair ,`(Zpos ,aa) ,`(Zneg
                  ,bb))))))))
        b))
     (lambda (a0)
     (Big.z_case
        ...
        (match (@ ggcd a0 b0)
           ((Pair g p)
             (match p
                ((Pair aa bb) `(Pair ,`(Zpos ,g) ,`(Pair ,`(Zneg ,aa) ,`(Zpos
                  ,bb))))))))
        (lambda (b0)
        (match (@ ggcd a0 b0)
           ((Pair g p)
             (match p
                ((Pair aa bb) `(Pair ,`(Zpos ,g) ,`(Pair ,`(Zneg ,aa) ,`(Zneg
                  ,bb))))))))
        b))
     a)))
...
(define signed_overflow (lambdas (wordsize_minus_one o1 o2)
  (let ((res (@ add1 o1 o2)))
    (match (@ andb (proj_sumbool (@ zle res (max_signed wordsize_minus_one)))
...
(define signed_overflow_with_carry (lambdas (wordsize_minus_one o1 o2 carry)
  (let ((c (@ bool_to_int wordsize_minus_one carry)))
    (let ((res (@ add1 (@ add1 o1 o2) (@ signed wordsize_minus_one c))))
      (match (@ andb
               (proj_sumbool (@ zle res (max_signed wordsize_minus_one)))
               (proj_sumbool (@ zle (min_signed wordsize_minus_one) res)))
         ((True) `(False))
         ((False) `(True)))))))
...
(define z_bin_decomp (lambda (x)
  (Big.z_case
     ...
     (lambda (p)
     (Big.positive_case
        (lambda (q) `(Pair ,`(True) ,`(Zpos
        ,q)))
        (lambda (q) `(Pair ,`(False) ,`(Zpos
        ,q)))
        (lambda (_) `(Pair ,`(True)
        ,`(Z0)))
        p))
     ...
     (Big.positive_case
        (lambda (q) `(Pair ,`(True)
        ,(@ sub1 `(Zneg ,q) `(Zpos ,`(XH)))))
        (lambda (q) `(Pair ,`(False) ,`(Zneg
        ,q)))
        (lambda (_) `(Pair ,`(True) ,`(Zneg
        ,`(XH))))
        p))
     x)))
...
(define set (lambdas (i v m)
  (match m
     ((Leaf)
       (Big.positive_case
          (lambda (ii) `(Node ,`(Leaf) ,`(None)
          ,(@ set ii v `(Leaf))))
          (lambda (ii) `(Node ,(@ set ii v `(Leaf)) ,`(None)
          ,`(Leaf)))
          (lambda (_) `(Node ,`(Leaf) ,`(Some ,v)
          ,`(Leaf)))
          i))
     ((Node l o r)
       (Big.positive_case
          (lambda (ii) `(Node ,l ,o
          ,(@ set ii v r)))
          (lambda (ii) `(Node ,(@ set ii v l) ,o
          ,r))
          (lambda (_) `(Node ,l ,`(Some ,v)
          ,r))
          i)))))
...
(define r0 (error "AXIOM TO BE REALIZED"))

(define r1 (error "AXIOM TO BE REALIZED"))

(define rplus (error "AXIOM TO BE REALIZED"))

(define rmult (error "AXIOM TO BE REALIZED"))

(define ropp (error "AXIOM TO BE REALIZED"))

(define rinv (error "AXIOM TO BE REALIZED"))

(define total_order_T (error "AXIOM TO BE REALIZED"))
...
(define p2R (lambda (p)
  (Big.positive_case
     ...
     (Big.positive_case
        (lambda (p0)
        (@ rplus r1 (@ rmult (@ rplus r1 r1) (p2R t))))
        (lambda (p0)
        (@ rplus r1 (@ rmult (@ rplus r1 r1) (p2R t))))
        (lambda (_)
...
(define bpow (lambdas (r e)
  (Big.z_case
     ...
     r1)
     (lambda (p)
     (z2R (@ pow_pos (radix_val r) p)))
     (lambda (p)
     (rinv (z2R (@ pow_pos (radix_val r) p))))
     e)))
...
(define shr_1 (lambda (mrs)
  (match mrs
     ((Build_shr_record m r s)
       (let ((s0 (@ orb r s)))
         (Big.z_case
            (lambda (_) `(Build_shr_record ,`(Z0) ,`(False)
            ,s0))
            (lambda (p0)
            (Big.positive_case
               (lambda (p) `(Build_shr_record ,`(Zpos ,p) ,`(True)
               ,s0))
               (lambda (p) `(Build_shr_record ,`(Zpos ,p) ,`(False)
               ,s0))
               (lambda (_) `(Build_shr_record ,`(Z0) ,`(True)
               ,s0))
               p0))
            (lambda (p0)
            (Big.positive_case
               (lambda (p) `(Build_shr_record ,`(Zneg ,p) ,`(True)
               ,s0))
               (lambda (p) `(Build_shr_record ,`(Zneg ,p) ,`(False)
               ,s0))
               (lambda (_) `(Build_shr_record ,`(Z0) ,`(True)
               ,s0))
               p0))
...
(define shr0 (lambdas (mrs e n)
  (Big.z_case
     (lambda (_) `(Pair ,mrs
     ,e))
     (lambda (p) `(Pair ,(@ iter p shr_1 mrs)
     ,(@ add1 e n)))
     (lambda (p) `(Pair ,mrs
     ,e))
     n)))
  ...
  (match (@ overflow_to_inf m s)
     ...
     ((False) `(F754_finite ,s
       ,(Big.z_case
           ...
           (lambda (p)
           `(XH))
           (@ sub1 (@ pow1 `(Zpos ,`(XO ,`(XH))) prec) `(Zpos ,`(XH))))
       ,(@ sub1 emax prec))))))
                   ...
                   (match p
                      ((Pair mz ez)
                        (Big.z_case
                           ...
                           `(F754_nan))
                           (lambda (mz0)
                           (@ binary_round_aux prec emax m (@ xorb sx sy) mz0
                             ez lz))
                           (lambda (p0)
                           `(F754_nan))
                           mz))))))))))))
...
(define load_mem_n (lambdas (seg addr nbytes_minus_one)
  (Big.nat_case
     ...
     (@ bind conv_monad (@ load_mem_n seg addr n) (lambda (rec)
       (@ bind conv_monad (@ load_Z size32 (of_nat1 `(S ,n))) (lambda (count)
         (@ bind conv_monad (@ arith size32 `(Add_op) addr count)
           ...
           (@ bind conv_monad (@ lmem seg p3) (lambda (nb)
             (@ bind conv_monad
               (@ cast_u
                 (@ minus
                   (@ mult (@ plus n `(S ,`(O))) `(S ,`(S ,`(S ,`(S ,`(S ,`(S
                     ,`(S ,`(S ,`(O)))))))))) `(S ,`(O)))
                 (@ minus
                   (@ mult (@ plus nbytes_minus_one `(S ,`(O))) `(S ,`(S ,`(S
                     ,`(S ,`(S ,`(S ,`(S ,`(S ,`(O)))))))))) `(S ,`(O))) rec)
               (lambda (p5)
               (@ bind conv_monad
                 (@ cast_u size8
                  ...
                  (Big.nat_case
                     ...
                     (Big.nat_case
                        ...
                        (Big.nat_case
                                 ...
                                 (Big.nat_case
                                    ...
                                    (Big.nat_case
                                       ...
                                       (Big.nat_case
                                          (lambda (_)
                                          (@ bind conv_monad
                                            (@ load_Z size8 `(Zpos ,`(XI
                                              ,`(XO ,`(XO ,`(XH))))))
                                            (lambda (modmask)
                                            (@ bind conv_monad
                                              (@ arith size8 `(Modu_op) p3
                                                modmask) (lambda (p4) no_op)))))
                                          (lambda (n6)
                                          (Big.nat_case
                                             (lambda (_)
                                             no_op)
                           ...
                           n0))
                        n))
                     (@ opsize (op_override pre) w)) (lambda (x)
                  (@ bind conv_monad
                    (@ cast_u size8
...
(define pos1
  (s2int80
    (@ append `(String ,`(Ascii ,`(False) ,`(False) ,`(False) ,`(False)
      ...
      (@ append `(String ,`(Ascii ,`(True) ,`(False) ,`(False) ,`(False)
        ,`(True) ,`(True) ,`(False) ,`(False)) ,`(String ,`(Ascii ,`(True)
        ,`(False) ,`(False) ,`(False) ,`(True) ,`(True) ,`(False) ,`(False))
        ,`(String ,`(Ascii ,`(True) ,`(False) ,`(False) ,`(False) ,`(True)
        ,`(True) ,`(False) ,`(False)) ,`(String ,`(Ascii ,`(True) ,`(False)
        ,`(False) ,`(False) ,`(True) ,`(True) ,`(False) ,`(False)) ,`(String
        ...
        (@ append `(String ,`(Ascii ,`(True) ,`(False) ,`(False) ,`(False)
          ...
          (@ append `(String ,`(Ascii ,`(False) ,`(False) ,`(False) ,`(False)
            ...
            (@ append `(String ,`(Ascii ,`(False) ,`(False) ,`(False)
                ...
                (@ append `(String ,`(Ascii ,`(False) ,`(False) ,`(False)
                  ...
                  (@ append `(String ,`(Ascii ,`(False) ,`(False) ,`(False)
                    ...
                    (@ append `(String ,`(Ascii ,`(False) ,`(False) ,`(False)
                      ...
                      ,`(String ,`(Ascii ,`(False) ,`(False) ,`(False)
                      ,`(False) ,`(True) ,`(True) ,`(False) ,`(False))
                      ,`(EmptyString))))))))) `(String ,`(Ascii ,`(False)
      ...
      (@ append `(String ,`(Ascii ,`(False) ,`(False) ,`(False) ,`(False)
        ...
        (@ append `(String ,`(Ascii ,`(True) ,`(False) ,`(False) ,`(False)
          ...
          (@ append `(String ,`(Ascii ,`(True) ,`(False) ,`(False) ,`(False)
            ...
            (@ append `(String ,`(Ascii ,`(False) ,`(False) ,`(False)
              ,`(False) ,`(True) ,`(True) ,`(False) ,`(False)) ,`(String
              ,`(Ascii ,`(True) ,`(False) ,`(False) ,`(False) ,`(True)
              ...
              (@ append `(String ,`(Ascii ,`(False) ,`(False) ,`(False)
                ...
                ,`(Ascii ,`(True) ,`(False) ,`(False) ,`(False) ,`(True)
                ,`(True) ,`(False) ,`(False)) ,`(EmptyString)))))))))
                (@ append `(String ,`(Ascii ,`(True) ,`(False) ,`(False)
                  ,`(False) ,`(True) ,`(True) ,`(False) ,`(False)) ,`(String
                  ,`(Ascii ,`(True) ,`(False) ,`(False) ,`(False) ,`(True)
                  ,`(True) ,`(False) ,`(False)) ,`(String ,`(Ascii ,`(False)
                  ,`(False) ,`(False) ,`(False) ,`(True) ,`(True) ,`(False)
...
(define empty0 (lambda (spaceSearch)
  (match spaceSearch
     ((Build_SpaceSearch empty1 single0 union bind1 search0) (empty1 __)))))

(define single (lambdas (spaceSearch x)
  (match spaceSearch
     ((Build_SpaceSearch empty1 single0 union bind1 search0)
       (@ single0 __ x)))))
...
(define result_rtl_state (run (@ add3 four six)))

(define inputs (@ single rosette four))
    ...
    (@ bind0 rosette inputs (lambda (n)
      (@ bind0 rosette inputs (lambda (m)
        (let ((s (run (@ add3 n m))))
          (match (fst s)
             ((Fail_ans)
               (@ single rosette `(Pair ,`(Pair ,n ,n) ,`(Inl ,`(Fail_ans)))))
             ((Trap_ans)
               (@ single rosette `(Pair ,`(Pair ,n ,n) ,`(Inl ,`(Trap_ans)))))
             ((Okay_ans u)
               (let ((r
                 (@ gp_regs (core (rtl_mach_state (snd result_rtl_state)))
                   `(EAX))))
