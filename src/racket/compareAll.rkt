...
(define instrList (string-trim (vector-ref (current-command-line-arguments) 0)))
(define instrs
  (map (lambda (line)
    (define input (map string-trim (string-split line ":")))
    (cons (first input) (second input)))
      (file->lines instrList)))

(define result (coqList->racketList (@ verifyInstrsEq (racketList->coqList instrs))))
